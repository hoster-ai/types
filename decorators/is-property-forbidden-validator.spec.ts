...
describe('IsPropertyForbidden decorator', () => {
  ...
  it('fails validation when forbidden property is present', async () => {
    ...
    expect(errors[0].constraints!.isPropertyForbidden).toBe(
      'forbiddenProp must not be present',
    );
  });
  ...
  it('uses default error message if no message is provided', async () => {
    const dto = plainToInstance(TestDto2, {
      someProp: 'allowed',
      forbiddenProp: 'not allowed',
    });
    const errors = await validate(dto);
    ...
    expect(errors[0].constraints).toBeDefined();
    expect(errors[0].constraints!.isPropertyForbidden).toBe(
      "Property 'forbiddenProp' is not allowed",
    );
  });
});
